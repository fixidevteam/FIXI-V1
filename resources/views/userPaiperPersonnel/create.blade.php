<x-app-layout :subtitle="'Ajouter un papier personnel'">
                ...
                <form method="POST" action="{{ route('paiperPersonnel.store') }}" class="space-y-6" enctype="multipart/form-data">
                    ...
                    <div class="shrink-0">
                        <img id='preview_img' class="h-16 w-16 object-cover rounded-full" src="{{ 
                                    session('temp_photo_perso') 
                                    ? (pathinfo(session('temp_photo_perso'), PATHINFO_EXTENSION) === 'pdf' 
                                        ? asset('./images/file.png') 
                                        : asset('storage/' . session('temp_photo_perso'))) 
